...
class Quantum_Hamiltonian:
    ...
    def __init__(self, natural_freq, rabi_freq):
    ...
    def rwa(self, driving_omega):
        ...
        """
        detuning = self.omega_0 - driving_omega
        return hbar*(1/2) * np.array([
            [-detuning, self.rabi_omega],
            [self.rabi_omega, detuning]
        ])
...
class Quantum_System:
    ...
    def evolve_state(self, initial_state, time, num_points, driving_freq, ham_type="rwa", **kwargs):
        ...
        def tdse(t, psi):
            if ham_type == "rwa":
                H = self.hamiltonian.rwa(2*np.pi*driving_freq)
            elif ham_type == "idle":
                H = self.hamiltonian.idle()
            elif ham_type == "non_rwa":
                H = self.hamiltonian.non_rwa(t, omega, **kwargs)
            elif ham_type == "custom":
                H = self.hamiltonian.custom_hamiltonian(kwargs)
            else:
