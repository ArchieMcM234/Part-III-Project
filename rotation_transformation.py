...
def rotating_transformation(state, omega, t):
    # this is for one qubit - but easily tensored to two?? what about in x and y directions?
    # rot_mat = np.array([[np.exp(1j * omega * t / 2), 0],
    #             [0, np.exp(-1j * omega * t / 2)]])

    # rot_mat = np.exp(0-Sx*(omega_0*t/2)*1j)

    # can get general form by having costheta I + i sin(theta) n.sigma

    rot_mat =  np.cos(effective_rabi*t / 2) * np.eye(2) - 1j * np.sin(effective_rabi*t / 2) * Sx

    rot_mat =  (np.cos(effective_rabi*t / 2) * np.eye(2) - 1j * np.sin(effective_rabi*t/ 2) * Sy)@ rot_mat
...
for a in range(num_steps):
    ...
    new_state = rotating_transformation(solution.y.T[a], omega, solution.t[a])

    x_expectation.append(new_state.conj() @ Sx @ new_state)
    y_expectation.append(new_state.conj() @ Sy @ new_state)
    z_expectation.append(new_state.conj() @ Sz @ new_state)

new_states = np.array([new_states])
print(new_states.T)
# Compute the Hermitian conjugates of the original vectors (transpose and conjugate)
hermitian_conjugates = new_states.conj()  # Shape will be (N, 2)

# Dot product between each Hermitian conjugate and transformed vector
# x_expectation = np.ei hermitian_conjugates, (Sx @ new_states).T)
# y_expectation = np.einsum('ij,ij->i', hermitian_conjugates, (Sy @ new_states).T)
# z_expectation = np.einsum('ij,ij->i', hermitian_conjugates, (Sz @ new_states).T)
