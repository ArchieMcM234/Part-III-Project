...
def rwa(natural_freqs_hz, rabi_freq_hz, driving_freq_hz, num_qubits, t):
    """
    Time-independent Hamiltonian in the rotating wave approximation (RWA).
    All frequencies should be in Hz.
    """
    natural_omegas = 2*np.pi*natural_freqs_hz
    rabi_omega = hz_to_radians(rabi_freq_hz)
    driving_omega = hz_to_radians(driving_freq_hz)
...
def ccd_rwa(natural_freq_hz, rabi_freq_hz, driving_freq_hz, phase_freq_hz, 
            phi_0, epsilon_m, theta_m, t):
    """
    Time-independent Hamiltonian in the RWA for CCD.
    All frequencies should be in Hz.
    """
    natural_omega = hz_to_radians(natural_freq_hz)
    rabi_omega = hz_to_radians(rabi_freq_hz)
    driving_omega = hz_to_radians(driving_freq_hz)
    phase_omega = hz_to_radians(phase_freq_hz)
...
def ccd_lab(natural_freq_hz, rabi_freq_hz, driving_freq_hz, phase_freq_hz,
            phi_0, epsilon_m, theta_m, t):
    """
...
def ccd_lab_multiple(natural_freqs_hz, rabi_freq_hz, driving_freq_hz, phase_freq_hz,
                     phi_0, epsilon_m, theta_m, t, coupling, num_qubits):
    """
    Time-dependent Hamiltonian in the lab frame for multiple qubits.
    All frequencies should be in Hz.
    """
    natural_omegas = hz_to_radians(natural_freqs_hz)
    rabi_omega = hz_to_radians(rabi_freq_hz)
    driving_omega = hz_to_radians(driving_freq_hz)
    phase_omega = hz_to_radians(phase_freq_hz)
    epsilon_m = hz_to_radians(epsilon_m)
...
def smart_lab(natural_freq_hz, rabi_freq_hz, driving_freq_hz, modulation_freq_hz, t):
    """
    ...
    rabi_omega = hz_to_radians(rabi_freq_hz)
    driving_omega = hz_to_radians(driving_freq_hz)
    modulation_omega = hz_to_radians(modulation_freq_hz)
...
def smart_rwa(natural_freq_hz, rabi_freq_hz, driving_freq_hz, modulation_freq_hz, t):
    """
